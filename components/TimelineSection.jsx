...
const TIMELINE_DATA = [
  {
    ...
    longDesc:
      "Form alignments of up to 3 architects. Early entrants receive priority slots.",
    status: "past",
    time: "18.00 pm",
  },
  {
...
export default function TimelineSection() {
  ...
  return (
    <section
      id="timeline"
      className="py-24 bg-slate-950/40 border-t border-slate-900/60 relative"
    >
          ...
          {TIMELINE_DATA.map((item, idx) => {
            ...
            return (
              <div key={item.id} className="relative group">
                ...
                <div
                  className={`absolute -left-[6px] top-3.5 h-3 w-3 rounded-full border transition-all duration-300 ${
                    item.status === "past"
                      ? "bg-slate-800 border-slate-700"
                      : item.status === "current"
                        ? "bg-blue-500 border-blue-400 shadow-[0_0_10px_rgba(251,191,36,0.4)] scale-125"
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                    <span className="sm:hidden font-varino text-xs text-blue-400 mb-1">
                      {item.date}
                    </span>
                  ...
                  <p className="text-slate-300 text-sm mt-2 font-normal leading-relaxed">
                    {item.shortDesc}
                  </p>
